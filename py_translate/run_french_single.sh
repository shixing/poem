...
for i in 1 2 3 4; do
echo $i
python run_french.py $model $source.single.nopunk.${i} $fsa1 $output.single.nopunck.${i} 1
cat $output.single.nopunck.${i} >> $output.single.nopunck.fsa
done

for i in 1 2 3 4; do
echo $i
python run_french.py $model $source.single.nopunk.${i} $fsa1 $output.single.nopunck.${i} 0
cat $output.single.nopunck.${i} >> $output.single.nopunck
done
