# usage: python compare_2fsa.py 10topics.txt 2fsa_results.txt
...
def main():
    ...
    for topic in topics:
        ...
        r = random.randint(1, 100000)
        fsa_path_1 = os.path.join(fsas_dir, "fsa.{}".format(r))
        fsa_path_2 = os.path.join(fsas_dir, "fsa2.{}".format(r))
        ...
        cmd = ["bash", "run-compare2fas-comma_or_not.sh", topic, fsa_path_1,fsa_path_2, source_path, rhyme_path, encourage_path]
    ...
    # generate the poem
        fsa_paths = [fsa_path_1, fsa_path_2]
        for i in xrange(len(fsa_paths)):
            fsa_path = fsa_paths[i]
            cmd = [rnn_path]
            ...
            fout.write("FSA_path: {}\n".format(i+1))
            if i == 1:
                fout.write("Ommit the comma and dot\n")
            fout.write("--------\n")
            fout.write(poem)
            fout.flush()
